...
1. [Dlaczego PostgreSQL?](https://claude.ai/chat/63358beb-c08e-4a75-8b00-c57d145ac993#1-dlaczego-postgresql)
2. [Instalacja krok po kroku](https://claude.ai/chat/63358beb-c08e-4a75-8b00-c57d145ac993#2-instalacja-krok-po-kroku)
3. [Pierwszy kontakt: Flask + PostgreSQL](https://claude.ai/chat/63358beb-c08e-4a75-8b00-c57d145ac993#3-pierwszy-kontakt-flask--postgresql)
4. [Typy relacji w bazach danych](https://claude.ai/chat/63358beb-c08e-4a75-8b00-c57d145ac993#4-typy-relacji-w-bazach-danych)
5. [Problem N+1 — Zabójca wydajności](https://claude.ai/chat/63358beb-c08e-4a75-8b00-c57d145ac993#5-problem-n1--zab%C3%B3jca-wydajno%C5%9Bci)
6. [Agregacje i statystyki](https://claude.ai/chat/63358beb-c08e-4a75-8b00-c57d145ac993#6-agregacje-i-statystyki)
7. [Transakcje — Wszystko albo nic](https://claude.ai/chat/63358beb-c08e-4a75-8b00-c57d145ac993#7-transakcje--wszystko-albo-nic)
8. [🚀 PROJEKT: System Rezerwacji Sal](https://claude.ai/chat/63358beb-c08e-4a75-8b00-c57d145ac993#8--projekt-system-rezerwacji-sal)
9. [Zadania do samodzielnego wykonania](https://claude.ai/chat/63358beb-c08e-4a75-8b00-c57d145ac993#9-zadania-do-samodzielnego-wykonania)
...
|SQLite|PostgreSQL|
|---|---|
|Notatnik z zamówieniami|Profesjonalny system kasowy|
|Działa offline|Działa przez sieć|
|Jeden użytkownik naraz|Tysiące użytkowników jednocześnie|
|Plik na dysku|Serwer bazy danych|
|Świetne do prototypów|Standard w produkcji|
...
VALUES ('Konferencja', '{"attendees": 150, "room": "A1", "catering": true}');
...
class Product(db.Model):
    ...
    price = db.Column(db.Numeric(10, 2), nullable=False)  # Lepsze niż Float dla pieniędzy!
...
product_tags = db.Table(
    ...
    db.Column('added_at', db.DateTime, default=db.func.now())  # Opcjonalnie: kiedy dodano
)
...
class Product(db.Model):
    ...
    tags = db.relationship(
        ...
        backref=db.backref(
            'products',           # Tag.products zwróci produkty z tym tagiem
            lazy=True
        )
    )
```
...
class Profile(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), unique=True)  # unique=True!
...
To najczęstszy błąd wydajnościowy w aplikacjach z ORM. Nazwa pochodzi od liczby zapytań SQL:
...
Transakcja to zestaw operacji, które muszą się wykonać **wszystkie albo żadna**. To podstawa bezpieczeństwa danych.
...
def apply_discount(percent):
    ...
    try:
        ...
        for product in products:
            ...
            if new_price < 1:
                raise ValueError(
                    f"Cena produktu '{product.name}' spadłaby poniżej 1 PLN!"
                )
...
Zbudujemy prawdziwy system rezerwacji sal konferencyjnych. Wykorzystamy wszystko, czego się nauczyliśmy!
...
**Podpowiedź:** Użyj `func.extract('dow', ...)` dla dnia tygodnia i `func.extract('hour', ...)` dla godziny.
...
**Podpowiedź:** Użyj `@event.listens_for(Booking, 'after_insert')` dla automatycznych powiadomień.
