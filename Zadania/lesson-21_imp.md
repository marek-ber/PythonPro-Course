...
Poniżej masz kompletny, minimalny setup Django formularza od zera, w poprawnej strukturze projektowej (Bootstrap + crispy + base + block content).
...
# 2. Utworzenie projektu i app

```bash id="a8k2sd"
django-admin startproject le21
cd le21
...
# 11. template formularza (CRUCIAL)
...
# 14. co tu się dzieje (model mentalny)

```text id="c1k8sd"
URL → view → form → template → base.html → Bootstrap + crispy → HTML
```

---

# 15. najważniejsze zasady

* `base.html` = global layout
* `{% block content %}` = miejsce na stronę
* `context` = dane
* `crispy` = render formularza
* Bootstrap = wygląd

---

Jeśli chcesz kolejny krok, mogę Ci zrobić:

* wersję z edit/update/delete (CRUD)
* albo wersję „produkcyjną” (CBV + Class Based Views + formularze + walidacja + messages framework)
